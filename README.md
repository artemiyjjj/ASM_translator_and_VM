...
Пример программы на разработанном диалекте языка:

``` Пример asm программы
    section .data:
...
- ```MachineWordInstruction``` - набор атрибутов как в ```StatementTerm``` с более жёсткими условиями (недопустимость значения None в ключевых полях).
...
- ```InstructionDecoder``` - hardwired (реализован методами класса ```ControlUnit``` на python).
- Генерирует все сигналы внутри машины.
- Метод ```execute_next_command``` моделирует выполнение одной очередной инструкции по всем циклам.

Сигналы:

- perform tick - инкремент счётчика тактов
...
1. Выборка команды
    ...
    Производится чтение из памяти следующей инструкции, находящейся по адресу, указанному в регистре ```PC```.

2. Выборка операнда

    Производится выборка операнда (загрузка в буфферный регистр) при наличии оного в представленной инструкции.

3. Выполнения команды

    Выполнение микрокоманд в зависимости от инструкции.

4. Проверка прерывания

    Проверка наличия и разрешения прерывания, при необходимости, проведение подготовки к прерыванию.

### Особенности работы машины

- На вход скрипта подаётся путь к файлу с сериализованным (транслированным) кодом программы.
- Машина должна быть инициализирована (с опциональными параметрами размера памяти и списком инициализированных устройств).
- Запуск симуляции происходит с помощью метода *simulation*, который требует десериализованного кода программы и опционально принимает расписание ввода/вывода и лимит счётчика машинных циклов.
- Шаг моделирования соответствует 1 такту работы машины.
- Статус машины во время шага моделирования выводится в журнал логирования.
- Логирование осуществляется с помощью стандартного модуля *logging*.
- Остановка моделирования происходит при:
    1. превышении лимита количества выполняемых инструкций
    2. исключении ```StopIteration``` -- если выполнена инструкция ```halt```.

### CLI

Представляет собой python-скрипт, работающий как консольное приложение - при запуске требуется 2 аргумента:

- файл с машинным кодом для исполнения.
- файл, с расписанием ввода/вывода.

## Тестирование

Разработаны модульные тесты для модулей транслятора и машины (см. [файлы тестов](/test/golden_tests/)), а так же интергационные тесты для всех вышеперечисленных и устройств ввода/вывода, представляющие собой процесс трансляции и выполнения минимального множества программ на машине.

Все тесты выполнены в формате Golden тестов.

Запуск тестов происходит при помощи комманды:

```pytest -v --continue-on-collection-errors```

Для запуска определенного файла с тестами:

```pytest -v --continue-on-collection-errors [путь к файлу с тестами]```

Обновить ожидаемые результаты Golden тестов в соответствии с фактическими результатами работы программы:
...
CI сконфигурирован для использования платформы GitLab. Создаётся Docker - образ, содержащий последнюю версию интерпретатора Python, образ загружается на сервер GitLab, происходит настройка окружения для проведения необходимых работ (загрузка необходимых пакетов командой *before_script*) и запускаются работы.
...
| Романов А. И. | cat        | 22  | 294       | 27          | 42     | 271   | asm | acc | neum | hw | tick | struct | trap | port | pstr | prob1 | spi |
| Романов А. И. | hello      | 20  | 364       | 39          | 122    | 733   | asm | acc | neum | hw | tick | struct | trap | port | pstr | prob1 | spi |
| Романов А. И. | hello_user | 87  | 2078      | 160         | 453    | 2748  | asm | acc | neum | hw | tick | struct | trap | port | pstr | prob1 | spi |
| Романов А. И. | prob1      | 64  | 1126      | 87          | 11881  | 70292 | asm | acc | neum | hw | tick | struct | trap | port | pstr | prob1 | spi |
```
